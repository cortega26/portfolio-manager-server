...
// ---------------------------------------------------------------------------
// Local DB-layer types (storage rows differ from domain API types)
// ---------------------------------------------------------------------------
...
interface CashPolicy {
  apyTimeline?: ApyEntry[];
  currency?: string;
  dayCount?: number;
  day_count?: number;
}

interface CashAccrualRow {
  month?: string;
  portfolio_id?: string | null;
...
function currencyDecimalPlaces(currency: string): number {
  const normalized = normalizeCurrency(currency);
  return CURRENCY_DECIMAL_PLACES[normalized] ?? 2;
}

function roundToCurrency(value: Decimal, currency: string): Decimal {
...
interface ClampPostingDayArgs {
  year: number;
  monthIndex: number;
  postingDay: 'last' | number;
}

function clampPostingDay({ year, monthIndex, postingDay }: ClampPostingDayArgs): number {
  if (postingDay === 'last') {
...
function resolveAccrualMonthKey(dateKey: string, postingDay: 'last' | number): string {
...
function resolvePostingDateForMonth(monthKey: string, postingDay: 'last' | number): string {
...
function normalizePortfolioId(value: unknown): string | null {
...
function matchesPortfolio(tx: TxRow, portfolioId: string | null | undefined): boolean {
...
export function toDateKey(date: string | Date | number | unknown): ISODate {
...
export function resolveApyForDate(
  ...
  let result = 0;
  for (const entry of timeline as ApyEntry[]) {
...
export function postInterestForDate(
  ...
  const effectiveDayCount =
    Number.isFinite(dayCount) && (dayCount ?? 0) > 0
      ? Math.round(dayCount ?? 0)
...
async function recordMonthlyAccrual({
  ...
  const rows = await storage.readTable<CashAccrualRow>('cash_interest_accruals');
  const normalizedCurrency = normalizeCurrency(currency);
...
export async function accrueInterest({
  ...
  policy?: CashPolicy;
  logger?: Logger;
  featureFlags?: { monthlyCashPosting?: boolean };
  postingDay?: 'last' | number;
  portfolioId?: string | null;
}): Promise<Record<string, unknown> | null> {
  const dateKey = toDateKey(date);
  await storage.ensureTable('transactions', []);
  const transactions = await storage.readTable<TxRow>('transactions');
  const currency = normalizeCurrency(policy?.currency);
  const timeline = Array.isArray(policy?.apyTimeline) ? policy.apyTimeline : [];
...
export async function postMonthlyInterest({
  ...
  await storage.ensureTable('cash_interest_accruals', []);
  const accruals = await storage.readTable<CashAccrualRow>('cash_interest_accruals');
  const normalizedPortfolioId = normalizePortfolioId(portfolioId);
...
export interface CashSeriesEntry {
  date: ISODate;
  rate: Decimal;
}
...
export function buildCashSeries({
  ...
  policy?: CashPolicy;
  from: string | Date | number | unknown;
  to: string | Date | number | unknown;
}): CashSeriesEntry[] {
  const start = new Date(`${toDateKey(from)}T00:00:00Z`);
  const end = new Date(`${toDateKey(to)}T00:00:00Z`);
  const result: CashSeriesEntry[] = [];
  const timeline = Array.isArray(policy?.apyTimeline) ? policy.apyTimeline : [];
  const dayCount = resolveDayCount(policy);
...
export function transactionIsExternal(tx: { type?: string }): boolean {
