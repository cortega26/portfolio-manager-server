...
function normalizeEmailDeliveryConfig(config) {
  ...
  const normalized = {
    ...
    subjectPrefix: normalizeOptionalString(raw?.subjectPrefix, '[Portfolio Manager]'),
    ...
    transport: {
      ...
      port: Number.isFinite(Number(transport?.port)) ? Number(transport.port) : 587,
  ...
  const inferredConfigured = Boolean(
    ...
    (normalized.transport.connectionUrl || normalized.transport.host)
  );
  return {
    ...normalized,
...
function buildSubject(notification, emailConfig) {
  const prefix = normalizeOptionalString(emailConfig?.subjectPrefix, '[Portfolio Manager]');
...
function computeNextRetryAt({ attemptedAt, attempts, retryPolicy }) {
  ...
  return buildIsoTimestamp(attemptedTimestamp + computeRetryDelayMs({ attempts, retryPolicy }));
}
...
function resolveRetryEligibilityAt(notification, retryPolicy) {
  ...
  const lastAttemptAt = normalizeOptionalString(notification?.delivery?.email?.lastAttemptAt, null);
  if (!lastAttemptAt) {
    return null;
  }
...
export function getSignalNotificationEmailDeliveryEligibility(
  ...
  if (nextRetryTimestamp === null || nowTimestamp === null || nextRetryTimestamp <= nowTimestamp) {
...
export function createSignalNotificationMailer({ config, logger, transport } = {}) {
  ...
  function ensureTransport() {
    ...
    if (!activeTransport) {
      activeTransport = nodemailer.createTransport(buildTransportOptions(emailConfig));
      ownsTransport = true;
    }
...
export async function requeueSignalNotificationEmailDelivery({
  ...
  if (!storage) {
    throw new Error('requeueSignalNotificationEmailDelivery requires a storage instance.');
  }
  const normalizedNotificationId = normalizeOptionalString(notificationId, null);
  if (!normalizedNotificationId) {
    throw new Error('requeueSignalNotificationEmailDelivery requires a notification id.');
  }
...
export async function processPendingSignalNotificationEmails({
  ...
  if (!storage) {
    throw new Error('processPendingSignalNotificationEmails requires a storage instance.');
  }
  ...
  try {
    return await withLock(buildDeliveryLockKey(storage), async () => {
        ...
        if (eligibility?.shouldExhaust) {
          await storage.upsertRow(
            ...
            buildUpdatedNotificationRow(notification, {
              ...
              nextRetryAt: null,
              exhaustedAt:
                normalizeOptionalString(notification?.delivery?.email?.exhaustedAt, null) ?? now,
            }),
            ['id']
          );
          exhausted += 1;
      ...
      for (const { notification } of eligibleNotifications) {
        ...
        const nextAttempts = normalizeAttempts(notification?.delivery?.email?.attempts) + 1;
        ...
        try {
          ...
          await storage.upsertRow(
            ...
            buildUpdatedNotificationRow(notification, {
              ...
              exhaustedAt: nextAttempts >= retryPolicy.maxAttempts ? attemptedAt : null,
              failure: {
