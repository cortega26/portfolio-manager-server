...
beforeEach(() => {
  ...
  writeFileSync(path.join(staticDir, 'asset.js'), "console.log('desktop-shell-asset');", 'utf8');
});
...
test('SPA shell serves index fallback without intercepting API routes', async () => {
  ...
  try {
    const rootResponse = await request(app).get('/').set('Accept', 'text/html').expect(200);
    ...
    await request(app).get('/missing.js').set('Accept', '*/*').expect(404);
  } finally {
    await closeApp(app);
  }
});
