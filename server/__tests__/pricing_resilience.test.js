...
test('signals preview degrades to fresh EOD prices when alpaca live quotes fail auth', async () => {
  ...
  assert.equal(
    latestQuoteCalls,
    1,
    'misconfigured live provider should be skipped after auth failure'
  );
  assert.equal(
    historicalCalls,
    1,
    'freshly cached EOD prices should avoid redundant upstream fetches'
  );
  await closeApp(app);
});
...
test('signals preview prefers a fresh historical close over a persisted close after a live quote failure during market hours', async () => {
  const today = new Date().toISOString().slice(0, 10);
  await seedLatestClose([{ ticker: 'MSFT', date: today, adj_close: 305.75 }]);
  ...
  const app = await createSessionTestApp({
    ...
    priceProvider: {
      async getDailyAdjustedClose() {
        historicalCalls += 1;
        return [{ date: today, adjClose: 315.25 }];
      },
    },
    config: {
      prices: {
  ...
  assert.equal(response.body.prices.MSFT, 315.25);
  assert.equal(response.body.pricing.symbols.MSFT.status, 'degraded');
  assert.equal(response.body.pricing.symbols.MSFT.source, 'historical');
...
test('signals preview prefers a fresh historical close over an older persisted close when the market is closed', async () => {
  const today = new Date().toISOString().slice(0, 10);
  await seedLatestClose([{ ticker: 'MSFT', date: '2024-01-02', adj_close: 305.75 }]);
  ...
  assert.deepEqual(response.body.errors, {});
  assert.equal(response.body.prices.MSFT, 315.25);
  assert.equal(response.body.rows[0].currentPrice, 315.25);
...
test('bulk latest pricing prefers a fresh historical close over a persisted close after a live quote failure during market hours', async () => {
  const today = new Date().toISOString().slice(0, 10);
  await seedLatestClose([{ ticker: 'MSFT', date: today, adj_close: 312.34 }]);
  ...
  const app = await createSessionTestApp({
    ...
    priceProvider: {
      async getDailyAdjustedClose() {
        historicalCalls += 1;
        return [{ date: today, adjClose: 333.45 }];
      },
    },
    config: {
      prices: {
  ...
  assert.deepEqual(response.body.errors, {});
  assert.equal(response.body.series.MSFT.length, 1);
  assert.equal(response.body.series.MSFT[0].close, 333.45);
  assert.equal(response.body.metadata.symbols.MSFT.status, 'degraded');
  assert.equal(response.body.metadata.symbols.MSFT.source, 'historical');
  assert.ok(response.body.metadata.symbols.MSFT.warnings.includes('LATEST_QUOTE_UNAVAILABLE'));
  assert.equal(historicalCalls, 1);
...
test('bulk latest pricing prefers a fresh historical close over an older persisted close when the market is unavailable', async () => {
  const today = new Date().toISOString().slice(0, 10);
  await seedLatestClose([{ ticker: 'MSFT', date: '2024-01-02', adj_close: 312.34 }]);
...
test('bulk latest pricing still rejects stale persisted closes beyond the freshness window', async () => {
  await seedLatestClose([{ ticker: 'MSFT', date: '2000-01-03', adj_close: 199.99 }]);
...
test('configured provider health skips yahoo after repeated upstream failures and prefers stooq', async () => {
  ...
  assert.equal(
    yahooCalls,
    2,
    'yahoo should stop being retried after hitting the unhealthy threshold'
  );
