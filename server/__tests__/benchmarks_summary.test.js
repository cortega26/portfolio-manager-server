...
test('benchmarks summary exposes matched-flow money weighted benchmarks and marks partial windows', async () => {
  ...
  const response = await request(app).get('/api/benchmarks/summary?from=2024-01-02&to=2024-06-28');
...
test('benchmarks summary returns null when a matched-flow benchmark cannot be calculated', async () => {
  ...
  await storage.writeTable('nav_snapshots', [
    ...
    },
    {
      ...
      ex_cash_nav: 1100,
      cash_balance: 150,
      risk_assets_value: 1100,
      stale_price: false,
    },
  ]);
  await storage.writeTable('transactions', [{ date: '2024-01-02', type: 'DEPOSIT', amount: 1000 }]);
  await storage.writeTable('prices', [
    { ticker: 'SPY', date: '2024-01-02', adj_close: 100 },
  ...
  const response = await request(app).get('/api/benchmarks/summary?from=2024-01-02&to=2025-01-10');
...
test('benchmarks summary includes max_drawdown with value, peak_date, and trough_date', async () => {
  ...
  const response = await request(app).get('/api/benchmarks/summary?from=2024-01-02&to=2024-01-04');
