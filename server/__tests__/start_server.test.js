...
test('resolveSchedulerEnabled honors explicit overrides before config', () => {
  assert.equal(resolveSchedulerEnabled(false, { jobs: { nightlyEnabled: true } }), false);
  assert.equal(resolveSchedulerEnabled(true, { jobs: { nightlyEnabled: false } }), true);
});

test('resolveSchedulerEnabled falls back to config when no override is provided', () => {
  assert.equal(resolveSchedulerEnabled(undefined, { jobs: { nightlyEnabled: false } }), false);
  assert.equal(resolveSchedulerEnabled(undefined, { jobs: { nightlyEnabled: true } }), true);
});
