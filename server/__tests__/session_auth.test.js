...
test('session auth mode requires the configured desktop session token', async () => {
  ...
  const missing = await request(app).get('/api/portfolio/desktop').expect(401);
...
test('session auth mode persists and retrieves portfolios without portfolio keys', async () => {
  ...
  const response = await withSession(request(app).get('/api/portfolio/desktop')).expect(200);
...
test('session auth mode supports custom header names', async () => {
  ...
  const response = await withSession(
    request(app).post('/api/portfolio/custom-header').send({ transactions: [] }),
    SESSION_TOKEN,
    'X-Desktop-Auth'
  );
