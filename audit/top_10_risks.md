...
| 3 | CQ-003 | S1 | Duplicate transaction UIDs are dropped silently, corrupting customer histories. | Fail validation on duplicates and surface actionable errors to clients. | API Owner |
...
| 5 | CQ-005 | S2 | N+1 price downloads hammer the backend and upstream providers during ROI fallback. | Introduce batched price endpoint or reuse monitoring cache for fallback. | Backend Lead |
...
| 7 | CQ-007 | S2 | Locale-insensitive share formatting causes user confusion and rounding errors. | Route share displays through `formatNumber` with asset-aware precision. | Frontend Lead |
