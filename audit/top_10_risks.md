...
| 6 | CQ-006 | S2 | JSON table rewrites scale poorly, risking timeouts and partial writes on large saves. | ✅ Upserts now journal to append-only logs with periodic compaction, keeping writes bounded while preserving atomic snapshots. | API Owner |
...
| 10 | CQ-010 | S2 | Minified `server/app.js` obstructs reviews and static tooling, slowing incident response. | ✅ Server bootstrap restored to formatted source with standard lint/format hooks. | Backend Lead |
