...
| `npm run docs:check`       | docs               | Fails when active docs reference missing npm scripts, missing repo paths, or known stale implementation claims.        |
...
| `npm run codacy:analyze`   | static analysis    | Runs the repo-root Codacy analysis flow, writes SARIF and summary reports, and fails on unresolved touched-file drift. |
| `npm run check:complexity` | structural drift   | Evaluates touched production files for unresolved Lizard findings and local duplication thresholds.                    |
| `npm run verify:docs`      | bootstrap + docs   | Runs the doctor, doc contract checks, and quality-gate contract together.                                              |
| `npm run verify:quality`   | repo health        | Runs the canonical anti-drift guardrail: docs, lint, typecheck, format, Codacy, complexity, build, and tests.          |
...
- `npm run verify:quality` is the repo-wide anti-drift gate for code health and tooling consistency.
