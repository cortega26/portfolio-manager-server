...
| Sensitive data in logs | ✅ Resolved (2025-11-11) | `pino-http` now redacts `X-Portfolio-Key`/`X-Portfolio-Key-New` via the shared config in `server/app.js`, and `server/__tests__/http_logging_redaction.test.js` asserts the masking behaviour. | Keep the header list up to date whenever new credential headers are introduced; extend the regression test alongside any logging changes. |
