...
export function errors(err, req, res, _next) {
  ...
  res
    .status(status)
    .json({ error: code, message: status < 500 ? err.message : 'Unexpected server error' });
}
app.use(errors);
```
...
export const validatePortfolio = (req, res, next) => {
  ...
  if (!p.success)
    return res.status(400).json({ error: 'VALIDATION_ERROR', details: p.error.issues });
  req.body = p.data;
  next();
};
```
...
app.get('/api/prices/:symbol', async (req, res) => {
  ...
  if (!data) {
    data = await fetchHistoricalPrices(req.params.symbol, req.query.range);
    cache.set(key, data);
  }
