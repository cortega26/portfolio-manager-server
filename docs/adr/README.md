...
| `001-decimal-js-for-finance.md`      | Accepted            | Canonical financial arithmetic uses `decimal.js`                           | `server/finance/decimal.ts`, `server/finance/portfolio.ts`, `server/finance/returns.ts`                                    |
| `002-sqlite-local-storage.md`        | Superseded by `008` | SQLite is the sole persistence layer                                       | `server/data/storage.js`, `server/data/portfolioState.js`                                                                  |
| `003-session-token-auth.md`          | Accepted            | Desktop auth uses a process-scoped session token plus local PIN            | `electron/main.cjs`, `electron/preload.cjs`, `server/plugins/sessionAuth.ts`, `server/auth/localPinAuth.ts`                |
...
| `008-json-table-storage-on-sqljs.md` | Superseded by `009` | The storage layer is `JsonTableStorage` persisted through `sql.js`         | `server/data/storage.js`, `server/data/portfolioState.js`, `server/utils/atomicStore.js`                                   |
| `009-native-sqlite-price-history.md` | Accepted            | Native SQLite (better-sqlite3) for prices table, all others stay on sql.js | `server/data/nativePriceStore.js`, `server/data/hybridStorage.js`, `server/data/storage.js`                                |
