...
export function buildDesktopRuntimeConfig({
  ...
  const normalizedSessionToken = normalizeString(sessionToken ?? API_SESSION_TOKEN);
  if (normalizedSessionToken) {
    config.API_SESSION_TOKEN = normalizedSessionToken;
  }
  const normalizedPortfolioId = normalizeString(activePortfolioId ?? ACTIVE_PORTFOLIO_ID);
  if (normalizedPortfolioId) {
    config.ACTIVE_PORTFOLIO_ID = normalizedPortfolioId;
  }
  const normalizedHeader = normalizeString(sessionAuthHeader ?? SESSION_AUTH_HEADER);
  if (normalizedHeader) {
    config.SESSION_AUTH_HEADER = normalizedHeader;
  }
  const normalizedTimeout = normalizeNumber(requestTimeoutMs ?? REQUEST_TIMEOUT_MS);
  if (normalizedTimeout) {
    config.REQUEST_TIMEOUT_MS = normalizedTimeout;
  }
  const normalizedNightlyActive = normalizeBoolean(jobNightlyActive ?? JOB_NIGHTLY_ACTIVE);
  if (normalizedNightlyActive !== undefined) {
    config.JOB_NIGHTLY_ACTIVE = normalizedNightlyActive;
  }
  const normalizedNightlyHour = normalizeNightlyHour(jobNightlyHourUtc ?? JOB_NIGHTLY_HOUR_UTC);
...
export function readDesktopRuntimeConfigFromArgv(argv = process.argv) {
  ...
  const encodedConfig = argv.find(
    (entry) => typeof entry === 'string' && entry.startsWith(DESKTOP_RUNTIME_CONFIG_ARG_PREFIX)
  );
  if (!encodedConfig) {
    return Object.freeze({});
  }
  return decodeDesktopRuntimeConfig(encodedConfig.slice(DESKTOP_RUNTIME_CONFIG_ARG_PREFIX.length));
}
