        ...
        <tbody className="divide-y divide-slate-200 dark:divide-slate-900">
          {breakdown.map((row) => (
            <tr key={row.month} className="bg-white dark:bg-slate-950">
              <td
                className={`px-3 font-semibold text-slate-700 dark:text-slate-200 ${compact ? 'py-1.5' : 'py-2'}`}
              >
                {row.label}
              </td>
      ...
      {timeline.map((item) => (
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <div>
              <p
                className={`font-semibold uppercase tracking-wide text-indigo-500 dark:text-indigo-300 ${compact ? 'text-[11px]' : 'text-xs'}`}
              >
                {item.dateLabel}
              </p>
              <h3
                className={`font-semibold text-slate-800 dark:text-slate-100 ${compact ? 'text-sm' : 'text-base'}`}
              >
                {item.title}
              </h3>
            </div>
          ...
          <p
            className={`text-slate-600 dark:text-slate-400 ${compact ? 'mt-2 text-xs' : 'mt-3 text-sm'}`}
          >
            {item.description ?? ''}
          </p>
        </li>
      ...
      >
        <header className="flex items-center justify-between">
          <div>
            ...
            <p
              className={`text-slate-500 dark:text-slate-400 ${compact ? 'mt-1 text-xs' : 'mt-1 text-sm'}`}
            >
