  ...
  const summaryCards = [
    {
      ...
      value:
        Number.isFinite(Number(summary?.totals?.totalCost))
    ...
    },
    {
      ...
      value:
        Number.isFinite(Number(summary?.totals?.totalRealised))
    ...
    },
    {
      ...
      value:
        Number.isFinite(Number(summary?.totals?.totalUnrealised))
    ...
    },
    {
      ...
      value:
        Number.isFinite(Number(summary?.totals?.totalNav))
          ...
          <div className="space-y-1">
            ...
            <dl className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
              ...
              <div>
                ...
                <dd className="inline font-medium text-slate-700 dark:text-slate-200">
                  {lastUpdatedAt ? formatDate(lastUpdatedAt, { dateStyle: "medium", timeStyle: "short" }) : "—"}
                </dd>
              </div>
              <div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        {!hasRows ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse text-sm" aria-label={t("prices.table.aria")}>
              ...
              <tbody>
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 align-top text-slate-600 dark:text-slate-300">
                      {row.shares !== null ? formatNumber(row.shares, { maximumFractionDigits: 9 }) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
                      {row.scope === "holding" && row.avgCost !== null ? formatCurrency(row.avgCost) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
                      {row.scope === "holding" && row.totalCost !== null ? formatCurrency(row.totalCost) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
                      {row.marketValue !== null ? formatCurrency(row.marketValue) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
                      {row.scope === "holding" && row.unrealised !== null ? formatCurrency(row.unrealised) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
                      {row.scope === "holding" && row.realised !== null ? formatCurrency(row.realised) : "—"}
                    </td>
                    <td className="px-4 py-3 align-top text-slate-900 dark:text-slate-100">
