...
describe("App ROI degradations", () => {
  ...
  it("keeps the last valid ROI snapshot when the ROI API temporarily fails", async () => {
    ...
    api.fetchDailyRoi
      ...
      .mockRejectedValueOnce(Object.assign(new Error("ROI service unavailable"), {
        name: "ApiError",
    ...
    assert.ok(
      await screen.findByText(/Stale ROI/i),
      "labels ROI status as stale",
    );
  ...
  it("shows ROI unavailable when the API fails before any valid snapshot exists", async () => {
    const api = await import("../utils/api.js");
    api.fetchDailyRoi.mockRejectedValueOnce(Object.assign(new Error("ROI service unavailable"), {
      name: "ApiError",
