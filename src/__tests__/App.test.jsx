...
describe('App tab navigation', () => {
  it('renders dashboard by default and shows validation when saving without an ID', async () => {
    render(<App />);
    assert.ok(await screen.findByText('Equity Balance'));
  ...
  it('surfaces empty states for the new analytics tabs', async () => {
    ...
    assert.ok(await screen.findByText(/Record transactions to see contribution trends/i));

    await userEvent.click(screen.getByRole('button', { name: /metrics/i }));
    assert.ok(await screen.findByText(/Metrics become available after you add holdings/i));
  ...
  it('allows undoing a transaction from the transactions table', async () => {
    ...
    assert.ok(await screen.findByText('No transactions recorded yet.'));
  });
});
