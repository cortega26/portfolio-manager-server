...
test('shows validation feedback when required fields are missing', () => {
  render(
    <TransactionsTab transactions={[]} onAddTransaction={() => {}} onDeleteTransaction={() => {}} />
  );
...
test('prevents withdrawals that would overdraw cash before persisting', () => {
  ...
  assert.ok(screen.getByText(/Withdrawal exceeds available cash/i), 'user sees validation error');
});
...
test('virtualizes large transaction lists, supports scrolling, and cooperates with filters', () => {
  ...
  try {
    ...
    assert.ok(screen.getByText(/SYM20/i), 'row near scroll offset becomes visible');
    ...
    const summary = screen.getByText(/Showing 1-1 of 1 transactions \(filtered from 1,200\)/i);
