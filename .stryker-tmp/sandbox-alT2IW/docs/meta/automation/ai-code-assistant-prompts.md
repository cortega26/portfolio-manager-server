...
No audit action items remain open; docs/reference/HARDENING_SCOREBOARD.md captures evidence for each phase and is now locked for maintenance.
Before coding, review AI_IMPLEMENTATION_PROMPT.md for guardrails, docs/reference/HARDENING_SCOREBOARD.md for historical context, and the existing Vitest/Playwright/performance harnesses in src/__tests__/, server/__tests__/, tools/, and e2e/ to ensure your changes preserve their guarantees.
Follow repository guardrails: keep coverage ≥90% on touched files, avoid shell=true, emit structured logs, and update docs/reference/HARDENING_SCOREBOARD.md + README if you adjust testing/CI expectations.
```
...
3. Update README.md and docs/reference/HARDENING_SCOREBOARD.md only if coverage expectations or commands change; otherwise note validation in your PR evidence.
...
Document any threshold or metric changes in README/testing-strategy guidance and refresh docs/reference/HARDENING_SCOREBOARD.md notes if expectations move.
...
2. Update README.md (Testing/CI sections) and docs/reference/HARDENING_SCOREBOARD.md if execution steps, prerequisites, or workflow snippets change.
...
- Confirm docs/reference/HARDENING_SCOREBOARD.md remains accurate (update only if expectations changed).
